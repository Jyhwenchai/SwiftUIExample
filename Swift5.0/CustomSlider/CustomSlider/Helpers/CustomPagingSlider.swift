...
struct CustomPagingSlider<Content: View, TitleContent: View, Item: RandomAccessCollection>: View where Item: MutableCollection, Item.Element: Identifiable {
            ...
            VStack(spacing: 0) {
              titleContent(item)
                ...
                .visualEffect { content, geometryProxy in
                  content
                    .offset(x: scrollOffset(geometryProxy, item: item.id as! UUID) )
                }
              content(item)
            }
  ...
  func scrollOffset(_ proxy: GeometryProxy, item: UUID) -> CGFloat {
    if let minX = proxy.bounds(of: .scrollView)?.minX {
      print("id: \(item), \(minX)")
    }
